        ...
        <Link to={`/event/lesson/${props.slug}`} className="group">
            ...
            <div className={`rounded border border-gray-500 p-4 mt-2 group-hover:border-green-500 ${isActiveLesson ? 'bg-green-500' : ''}`}>
                <header className="flex items-center justify-between">
                    ...
                    <span className={`text-xs rounded py-[0.125rem] px-2 text-white border-green-300 font-bold ${isActiveLesson ? 'border-white' : ' '}`}>
                        {props.type === 'live' ? 'AO VIVO' : 'AULA PRÁTICA'}
                    </span>
                </header>

                <strong className={`text-white mt-5 block ${!isActiveLesson ? 'text-gray-200' : ''}`}>
                    {props.title}
                </strong>
            </div>
        </Link>
